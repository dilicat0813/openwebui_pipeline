    ...
    def pipe(
        ...
        if body.get("title", False):
            ...
            for query in [user_message]:
                ...
                r = requests.get(
                    f"https://ko.wikipedia.org/w/api.php?action=opensearch&search={query}&limit=1&namespace=0&format=json"
                )
            ...
            if len(titles) > 0:
                r = requests.get(
                    f"https://ko.wikipedia.org/w/api.php?format=json&action=query&prop=extracts&exintro&explaintext&redirects=1&titles={'|'.join(titles)}"
                )
                response = r.json()
                # get extracts
